...
(function() {
  ...
  function showSystems(tab) {
    ...
    if (tab==='equip') {
      ...
      html+='<h3>🎫兑换码</h3>';
      html+='<div class="redeem-input"><input type="text" id="redeem-input" placeholder="输入兑换码" maxlength="20" style="width:55%;padding:8px;border:1px solid var(--border);border-radius:4px;background:var(--bg);color:var(--text);font-size:13px;" /><button class="redeem-btn" onclick="window._ac.redeemCode(document.getElementById(\'redeem-input\').value)" style="padding:8px 12px;margin-left:4px;border-radius:4px;border:1px solid var(--accent);background:var(--bg-light);color:var(--accent);font-size:12px;font-weight:600;cursor:pointer;">兑换</button></div>';
      html+='<div class="redeem-list">';
      for (var code in REDEEM_CODES) { var info = REDEEM_CODES[code]; var used = state.redeemedCodes.includes(code); html+='<div class="redeem-item '+(used?'used':'')+'"><span class="redeem-code">'+(used?'✅':'🎫')+' '+info.name+'</span><span class="redeem-desc">'+info.desc+'</span></div>'; }
      html+='</div>';
      if (state.redeemedCodes.length > 0) html+='<p class="hint" style="margin-top:8px;">已使用'+state.redeemedCodes.length+'个兑换码</p>';
    }
    ...
    setTimeout(function() {
      try {
        var diag = {
          ...
          boardDisplay: document.getElementById('board') ? getComputedStyle(document.getElementById('board')).display : 'no-board',
          boardHeight: document.getElementById('board') ? getComputedStyle(document.getElementById('board')).height : 'no-board',
          aspectRatio: typeof CSS !== 'undefined' && CSS.supports ? CSS.supports('aspect-ratio', '1') : 'unknown',
          ...
          equipH: document.getElementById('equip-bar').offsetHeight,
