...
(function() {
  ...
  function renderEquipBar() { const el=document.getElementById('equip-bar'); if(!el) return; const pe=state.pet?PETS[state.pet].emoji:'🐾'; const we=state.wing&&state.wing!=='w0'?WINGS[state.wing].emoji:'🪽'; const gc=state.gems; el.innerHTML=`<div class="equip-icons"><span class="equip-icon" onclick="window._ac.showSystems('pet')">${pe}</span><span class="equip-icon" onclick="window._ac.showSystems('wing')">${we}</span><span class="equip-icon" onclick="window._ac.showSystems('equip')">🎒${state.inventory.length}</span><span class="equip-icon" onclick="window._ac.showSystems('gem')">💎${gc.red+gc.blue+gc.green+gc.yellow+gc.purple}</span></div>`; }
